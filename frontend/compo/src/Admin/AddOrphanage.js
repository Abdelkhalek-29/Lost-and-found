...
function AddOrphanage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (userInfoString) {
      ...
      try {
        const response = await axios.post('https://lost-seven.vercel.app/admin/adddar', formData, {
          headers: {
            token: `task__${token}`
          }
        });
        ...
        if (response.status === 200 && response.data.success) {
          console.log('Orphanage account added successfully:', response.data);
          setSuccessMessage('Orphanage account added successfully');
        }
      } catch (error) {
        console.error('Error sending report:', error);
        setErrorMessage('An error occurred while adding Orphanage account. Please try again later.');
      }
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <h3 className='H3Orphanage'>Add Orphanage Account</h3>
      <div className="form-groupOrphanage">
        <label>Name:</label>
        <input
          type="text"
          name="name"
